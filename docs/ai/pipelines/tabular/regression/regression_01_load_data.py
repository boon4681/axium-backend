...
class RegressionDataLoader:
    ...
    def load_sample_dataset(self, dataset_name: str = "california_housing") -> Tuple[pd.DataFrame, pd.Series]:
        ...
        if dataset_name == "california_housing":
            ...
            print(
                f"Target range: [{self.target.min():.2f}, {self.target.max():.2f}]")

            return self.data.copy(), self.target.copy()

        else:
            raise ValueError(
                f"Dataset '{dataset_name}' not supported. Choose from ['california_housing', 'diabetes', 'synthetic']")
    ...
    def load_csv_data(self, file_path: str, target_column: str,
        ...
        if not pd.api.types.is_numeric_dtype(self.target):
            print(
                f"Warning: Target column '{target_column}' is not numeric. Attempting conversion...")
            try:
                self.target = pd.to_numeric(self.target, errors='coerce')
                if self.target.isnull().any():
                    print(
                        f"Warning: {self.target.isnull().sum()} target values could not be converted to numeric")
            except:
                raise ValueError(
                    f"Target column '{target_column}' cannot be converted to numeric")
    ...
    def create_time_series_features(self, df: pd.DataFrame,
        ...
        df_ts['is_weekend'] = (
            df_ts[date_column].dt.dayofweek >= 5).astype(int)
        ...
        for col in numeric_cols:
            df_ts[f'{col}_rolling_mean_3'] = df_ts[col].rolling(
                window=3).mean()
